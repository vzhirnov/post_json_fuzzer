...
def get_all_combinations(matched_items: dict):
    keys, values = zip(
        *matched_items.items()
    )  # TODO: assert(no assert but IF) if no ()s with strategy, just plain dict values
    ...
    for dict_items in experiments:
        ...
        for key, value in dict_items.items():
            ...
            if key in tmp_d.keys():  # 1st case
                ...
                need_key = list(tmp_d.keys())[
                    list(tmp_d.values()).index(key)
                ...
                left_index, right_index = get_indexes_of_nearest_brackets(
                    tmp_d, key_index_in_d_base
                )
                key = tmp_d[left_index:right_index]
                tmp_d = eval(tmp_d)
                fuzz_item.update(
                    {value: key}
                )  # TODO: Improvement: replace key with correct path to value
            else:
