...
from src.utils.console_widgets import show_start_fuzz_info, show_fuzz_results_brief, add_line_separator
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url_aim, json=json_params, headers=hdrs, ssl=False, timeout=10000000) as response:
            got_suspicious_reply = \
                {'suspicious_reply': True} if response.status in suspicious_replies else {'suspicious_reply': False}
            response_body = await response.text()
            return response, json_params, response_body, got_suspicious_reply


async def start_fuzz(jsons):
    request_tasks = [post(url, json_params[0], headers, json_params[1]) for json_params in jsons]
    responses_bundle = [await f for f in tqdm.tqdm(asyncio.as_completed(request_tasks), total=len(request_tasks))]
...
if __name__ == '__main__':
    ...
    for response in actual_responses:  # TODO replace tuples with named tuple
        result_to_save[(response[0].status, response[0].reason)] = \
            [response[1]] if result_to_save.get((response[0].status, response[0].reason)) is None \
                else result_to_save[(response[0].status, response[0].reason)] + [response[1]]
